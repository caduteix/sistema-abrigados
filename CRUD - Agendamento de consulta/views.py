# Arquivo: CRUD - Agendamento de Consultta/views.py
...
# Definição dos status de agendamento. Usei uma lista simples.
...
class AgendamentoWidgets(param.Parameterized):
    # Widgets de Cadastro/Atualização
    id_consulta_input = pn.widgets.IntInput(name='ID Agendamento (para Atualizar/Excluir)', value=0, width=200)
    pessoa_abrigada_select = pn.widgets.Select(name='Pessoa Abrigada', options=get_pessoas_abrigadas_for_select(), width=300)
    servico_select = pn.widgets.Select(name='Serviço', options=get_servicos_for_select(), width=300)
    ...
    # Widgets de Filtro/Consulta
    filter_id_consulta_input = pn.widgets.IntInput(name='Filtrar por ID', value=0, width=150)
    filter_pessoa_select = pn.widgets.Select(name='Filtrar por Pessoa', options=get_pessoas_abrigadas_for_select(), width=250)
    filter_servico_select = pn.widgets.Select(name='Filtrar por Serviço', options=get_servicos_for_select(), width=250)
    ...
    def __init__(self, **params):
        ...
        self.update_agendamentos_display() # Carrega a tabela na inicialização
    ...
    def _on_agendar_consulta(self, event):
        try:
            ...
            if (self.pessoa_abrigada_select.value == 0 or self.servico_select.value == 0 or
                ...
                pn.pane.Alert('Erro: Preencha todos os campos obrigatórios (Pessoa Abrigada, Serviço, Data, Horário, Profissional).', alert_type='warning').show()
                return
            ...
            success = add_consulta(
                ...
                data_agendamento=str(self.data_agendamento_picker.value), # Converte para string YYYY-MM-DD
                horario_agendamento=self.horario_agendamento_picker.value, # Já está no formato HH:MM
            ...
            if success:
                ...
                self._on_limpar_campos(None) # Limpa os campos
                self.update_agendamentos_display() # Atualiza a tabela
            else:
                pn.pane.Alert('Não foi possível agendar a consulta. Verifique o console para detalhes.', alert_type='danger').show()
    ...
    def _on_atualizar_agendamento(self, event):
        try:
            ...
            if success:
                ...
                pn.pane.Alert('Não foi possível atualizar o agendamento. Verifique o console para detalhes.', alert_type='danger').show()
        except Exception as e:
    ...
    def _on_excluir_agendamento(self, event):
        try:
            ...
            if success:
                ...
                self._on_limpar_campos(None) # Limpa o ID
                self.update_agendamentos_display() # Atualiza a tabela
            else:
                pn.pane.Alert('Não foi possível excluir o agendamento. Verifique o console para detalhes.', alert_type='danger').show()
        except Exception as e:
    ...
    def update_agendamentos_display(self, query_str=None):
        # Esta função é chamada na inicialização e após operações CRUD
        if query_str is None:
            # Consulta padrão para exibir todos os agendamentos
            df_agendamentos = get_all_consultas()
        else:
            # Lógica para aplicar filtros (ainda não implementada no crud_operations.py de forma flexível)
            # Por enquanto, get_all_consultas() retorna tudo.
            # Para filtros, você precisaria de uma função get_filtered_consultas no crud_operations.py
            # que aceitasse os parâmetros de filtro e construísse a query SQL dinamicamente.
            df_agendamentos = get_all_consultas() # Temporário, até implementar filtros no crud_operations.py

        if df_agendamentos.empty:
            agendamentos_table_pane.objects = [pn.pane.Markdown("### Nenhum agendamento encontrado.")]
        else:
            # Garante que as colunas de data/hora sejam formatadas para exibição no Tabulator
            ...
            tabulator = pn.widgets.Tabulator(
                ...
                pagination='remote', # ou 'local' se preferir carregar tudo de uma vez
                page_size=10,
                disabled=True, # Impede edição direta na tabela
                selectable='checkbox' # Permite seleção de linhas
            )
    ...
    def _on_consultar_agendamento(self, event):
        # Esta função precisa construir uma query baseada nos filtros e passá-la para update_agendamentos_display
        # No momento, crud_operations.py não tem uma função de filtro genérica.
        # Para implementar isso, você precisaria adicionar uma função como `get_filtered_consultas`
        # em crud_operations.py que aceitasse os parâmetros de filtro e construísse a query SQL.

        # Exemplo de como você construiria os parâmetros para uma função de filtro:
        filters = {}
        ...
        # Por enquanto, apenas atualiza a exibição completa.
        # Você substituiria isso por uma chamada como:
        # self.update_agendamentos_display(query_params=filters)
    ...
    def _on_reset_filters(self, event):
        ...
        self.update_agendamentos_display() # Recarrega a tabela sem filtros


    def _on_tabulator_row_selected(self, event):
        # Verifica se há um Tabulator no agendamentos_table_pane.objects
        if agendamentos_table_pane.objects and isinstance(agendamentos_table_pane.objects[0], pn.widgets.Tabulator):
            ...
            if tabulator.selection: # Se alguma linha foi selecionada
                selected_indices = tabulator.selection
                # Pega a primeira linha selecionada (Panel Tabulator retorna índices baseados no DataFrame original)
                ...
                # Encontrar o ID da pessoa e serviço a partir do nome
                # Isso é importante porque o select espera o ID, não o nome
...
def agendamento_app():
    ...
    # Layout para Cadastro/Atualização/Exclusão
    ...
    # Layout para Filtros de Consulta
    ...
    main_agendamento_column = pn.Column(
        ...
        agendamentos_table_pane, # A tabela será atualizada dinamicamente aqui
        sizing_mode="stretch_width"
    )
